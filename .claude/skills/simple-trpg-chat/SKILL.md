...
## Known Pitfalls
1. New components often created but **not rendered** in RoomClient — always check `<NewPanel />` exists in JSX
2. `package-lock.json` removed — use **pnpm-lock.yaml** only
3. `better-sqlite3` requires C++20 compiler (GCC 11+ on Linux)
4. SSE privacy filter V3.15: targeted messages (`targetUserId` set) exclude host unless host is sender/target
5. `db:push` needed after any schema.ts change — Turbopack doesn't auto-sync SQLite
6. `instrumentation.ts` required for `initDb()` to run on startup
