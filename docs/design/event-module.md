...
> 状态:**已实现**(2026-07-24,v0.10.0,分支 `feat/event-module`)。产品决策见 §9(Joycai 已拍板)。
> 实现与本方案的主要一致点:双表 `story_events` / `story_event_visibility`;公开卡片仅广播元数据、
> 正文走受控拉取;锁定/解锁由客户端「可见 id 集合」实时判定(SSE `events_updated` 刷新);撤回删旧卡
> 重发「已撤回」卡以绕开客户端按 id 去重。偏差:新增 `card_message_id` 列(便于撤回/删除时定位公共卡片);
> DB 变更以幂等 DDL 直接应用(drizzle-kit push 卡在既有 `ai_token_usages` 交互式提示,与本功能无关)。
